...
vtkStandardNewMacro(ttkPathCompressionLocal);

ttkPathCompressionLocal::ttkPathCompressionLocal() {
  this->setDebugMsgPrefix("PathCompressionLocal");
  SetNumberOfInputPorts(1);
  SetNumberOfOutputPorts(1);
}

int ttkPathCompressionLocal::FillInputPortInformation(int port,
                                                 vtkInformation *info) {
  if(port == 0) {
...
int ttkPathCompressionLocal::FillOutputPortInformation(int port,
                                                  vtkInformation *info) {
  if(port == 0) {
...
int ttkPathCompressionLocal::dispatch(const SimplexId *const inputOrderArray,
  ...
  if(ret != 0)
    return !this->printErr("PathCompressionLocal.execute() error");

  return ret;
}

int ttkPathCompressionLocal::RequestData(vtkInformation *ttkNotUsed(request),
