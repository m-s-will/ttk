#include <ttkExTreeMUnstructured.h>
...
vtkStandardNewMacro(ttkExTreeMUnstructured);

ttkExTreeMUnstructured::ttkExTreeMUnstructured() {
...
int ttkExTreeMUnstructured::FillInputPortInformation(int port,
                                                     vtkInformation *info) {
  if(port == 0) {
...
int ttkExTreeMUnstructured::FillOutputPortInformation(int port,
                                                      vtkInformation *info) {
  switch(port) {
    case 0:
    case 1:
...
int ttkExTreeMUnstructured::getMergeTree(
...
int ttkExTreeMUnstructured::RequestData(vtkInformation *,
  ...
  this->printMsg("#Edges in original domain: "
  ...
  // compute path compression
  {
    ...
    ttk::PathCompression::OutputManifold om{
      ...
      ttkUtils::GetPointer<ttk::SimplexId>(descendingManifold), nullptr};
  ...
  std::array<std::vector<ttk::SimplexId>, 4> criticalPoints;
  {
    ...
    status
      = subModule.mergeCriticalPointVectors(criticalPoints, criticalPoints_);
    if(!status)
      return 0;
  }
