...
namespace ttk {
  ...
  public:
    ...
    int gatherNeighbourhood(std::set<IT> &toTraverse,
                            const TT *triangulation,
                            const int neighborhoodSize) const {
      std::set<IT> used;
    ...
    template <typename IT, typename TT>
    int computeMapCombinatorialDistribution(
      std::vector<std::map<IT, int>> &matchMap,
      TT *triangulation,
      ...
      for(IT i = 0; i < nFeatures0; i++) {
        ...
        gatherNeighbourhood<IT, TT>(
          toTraverse, triangulation, neighborhoodSize);

        // go through the vertices in the neigbourhood and check what manifolds
        // they fall in, also calculate the statistic U = sum(order) -
        // neighborhoodN(neighborhoodN + 1) / 2
        double U0 = 0;
        for(auto &vertex : toTraverse) {
        ...
        for(auto &m : matchMap[i]) {
          ...
          gatherNeighbourhood<IT, TT>(
            neighbourhood, triangulation, neighborhoodSize);
          double U1 = 0;
    ...
    template <typename IT, typename TT>
    int computeMapCombinatorial(std::vector<std::map<IT, int>> &matchMap,
                                TT *triangulation,
                                const IT *segmentation,
                                const IT *criticalPointVertexIds0,
                                const IT nFeatures0,
    ...
    template <typename IT, typename TT>
    int computeMapDistance(std::vector<std::map<IT, int>> &matchMap,
                           TT *triangulation,
                           const IT *segmentation,
                           const IT *criticalPointVertexIds0,
                           const IT nFeatures0,
                           const int neighborhoodSize,
                           const float neighbourhoodDistance) const {
      for(IT i = 0; i < nFeatures0; i++) {
        std::set<IT> toTraverse;
        std::set<IT> used;
        float pFeature[3] = {0.0, 0.0, 0.0};
        triangulation->getVertexPoint(
          criticalPointVertexIds0[i], pFeature[0], pFeature[1], pFeature[2]);
        std::string msg = "Feature pos: (" + std::to_string(pFeature[0]) + ", "
                          + std::to_string(pFeature[1]) + ", "
                          + std::to_string(pFeature[2]) + ")\n";
        // add feature to toTraverse
        toTraverse.insert(criticalPointVertexIds0[i]);
          ...
          for(auto &vertex : toTraverse) {
            ...
            if(res.second) {
              ...
              if(s != neighborhoodSize) {
                // add neighbours to toTraverse unless we are at the last
                // iteration
                const IT nNeighbors
                  = triangulation->getVertexNeighborNumber(vertex);
                for(int n = 0; n < nNeighbors; n++) {
    ...
    int computeManifoldSize(std::map<IT, int> &sizeMap,
                            const IT *segmentation,
    ...
    template <typename IT>
    int computeMapManifoldOverlap(std::map<IT, std::map<IT, int>> &matchMap,
                                  const IT *segmentation0,
                                  const IT *segmentation1,
                                  const IT nVertices) const {
...
#pragma omp parallel for num_threads(this->threadNumber_)
      ...
      for(IT i = 0; i < nFeatures0; i++) {
        ...
        for(auto &m : matches) {
          ...
          if(res != map1.end()) {
            matrix[indexFunction(i, res->second, nFeatures0, nFeatures1)]
              = m.second;
          }
        }
      }
    ...
    template <typename IT, typename IF>
    int computeSimilarityManifoldMatrix(
      double *matrix,
      ...
      for(IT i = 0; i < nFeatures0; i++) {
        ...
        for(auto &m : innerMap) {
          ...
          if(res != map1.end()) {
            ...
            matrix[indexFunction(i, res->second, nFeatures0, nFeatures1)]
              = double(m.second) / (size0);
            // if (vertex == 21722 && m.first == 21484) {
            //   this->printMsg("Intersection: " + std::to_string(m.second) +
            //   "sizes: size0=" + std::to_string(size0) + " size1=" +
            //   std::to_string(size1) + "\n");
            // }
          }
        }
