...
int main(int argc, char **argv) {
  ...
  for(int i = 0; i < repetitions; i++) {
    // change this to the pipeline which is done in ttkExTreeM, but without
    // Myimplicittriangulation
    auto exTreeM = vtkSmartPointer<ttkExTreeMUnstructured>::New();

    exTreeM->SetInputDataObject(0, inputDataObject);

    exTreeM->SetInputArrayToProcess(0, 0, 0, 0, inputArrayName.data());

    exTreeM->SetUseAllCores(false);
    exTreeM->SetThreadNumber(threadNumber);
    exTreeM->Modified();
    exTreeM->Update();
  }
